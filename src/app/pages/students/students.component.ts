...
Approve(data:AdvisorAssignments) {
  this.studentservice.approveRequest(data).subscribe(
    res=>{
...
constructor(private router:Router,private studentservice: StudentsService,private storage:AppstorageService){}
  ngOnInit(): void {
...
getAdvisorstudents(){
  ...
  this.studentservice.getAdvisorstudents(this.user.user.advisor).subscribe(
    res=>{
